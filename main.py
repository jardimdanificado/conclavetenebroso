...
def menu(options, icons, functions, intro):
	...
	while True:
		clear()
		print("\033[33m♔\033[0m  O Conclave Tenebroso: Zumbi Contra a Sombra do Império  \033[33m♔\n\033[0m")
		print(intro)
		for i in range(len(options)):
			if i == current:
				print('☞ \033[47m\033[30m' + icons[i] + "\033[47m\033[30m" + options[i] + "\033[0m")
			else:
		...
		if key == '\n':
			...
			if arrow_key == 'A' and current > 0:  #up
				current -= 1
			elif arrow_key == 'B' and current < len(options) - 1:  #down
				current += 1
...
dificuldade = menu([' facil', ' normal', ' dificil', ' tenebroso', "sair"],
                   ['\033[32m♟\033[0m',"\033[36m♜\033[0m","\033[33m♞\033[0m","\033[31m☠\033[0m",''],
...
map_size = menu(['pequeno', 'normal', 'grande', "enorme", "monstruoso", "colossal", "mundo" "sair"],
                   ['','','','','','','','',''],
				   [pequeno, normal, grande, enorme, monstruoso, colossal, mundo, exit],
...
creature_types = ["rainha", "rei"] + ["torre"] * 5 + ["bispo"] * 20 + ["cavalo"] * 5 + ["peao"] * 20
...
for i in range(math.floor((map_size * map_size) / dificuldade)):
	...
	rx = random.randint(0, map_size - 1)
	if (map[rx][ry] == '.'):
		creatures.append(Creature(creature_types[random.randint(0, len(creature_types)-1)], rx, ry))
while True:
	maprint()
	char = getch.getch()
	if char == 'q' or char == 'Q':
